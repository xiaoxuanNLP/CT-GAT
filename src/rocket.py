...
def attack(model, orig_test, tokenizer, ATTACK_ITER, task_name):
    ...
    orig_test = random.sample(orig_test, 3000)
    ...
    if len(correct_samples) > 1200:
        correct_samples = random.sample(correct_samples, 500)

    print("correct_samples = ", len(correct_samples))
    ...
    if task_name == "enron" or task_name == "assassin":
        ...
        ch = "reuters"
        attack_ori_only = True
    elif task_name == "jigsaw" or task_name == "HSOL" or task_name == "tweets" or task_name == "Founta":
        # print("fakereview")
        begsl = 0
        endsl = 180
        attack_totalnum = 180
            ...
            while cur_iter < ATTACK_ITER and len(cache_pos_li) < maxsl * 0.98 and len(cache_pos_li) < attack_totalnum:
                ...
                for e in range(attack_batchsize * epoch + int(0.05 * len(cache_pos_li))):  # 2  #5
                    ...
                    for i in range(attack_batchsize):  # 3  #2
                        ...
                        target_word = words_li[word_pos[i]]  # 停用词的处理就是直接skip
...
if __name__ == '__main__':
    ...
    parser.add_argument('--limit', default=550, type=int)
    args = parser.parse_args()

    pipe(args.name, args.limit)
    print("-----------------" + args.name + "+" + str(args.limit) + "-------------")
